...
class MindstormsGadget(AlexaGadget):
    ...
    def _activate(self, command, speed=50):
        ...
        if command in Command.SORT.value:
            ...
            """
            #self.lift.on_for_rotations(SpeedPercent(20), -2)
            ...
            #logger.info("Color is {}".format(self.cs.value()))
            # self._send_event(EventName.SPEECH,{'speechOut': "Event recieve"})
            ...
            if(int (self.cs.value())==5):
                ...
                logger.info("WHITE")
            elif(int (self.cs.value())==3 or int (self.cs.value()) == 4):
                logger.info("YELLOW")
            else:
                logger.info("UNKNOWN")
            ...
            while True:
                sVal = "Color is {}",self.cs.value()
                print(sVal)
                self._send_event(EventName.SPEECH,{'speechOut': sVal})
                time.sleep(0.3)
            """
